...
object Storage {
  ...
  fun getPrefix(ctx: Context) =
    ctx.getSharedPreferences(PREF, Context.MODE_PRIVATE).getString(KEY_PREFIX, "prise-") ?: "prise-"
  fun setPrefix(ctx: Context, v: String) =
    ctx.getSharedPreferences(PREF, Context.MODE_PRIVATE).edit().putString(KEY_PREFIX, v).apply()

  fun getTheme(ctx: Context) =
    ctx.getSharedPreferences(PREF, Context.MODE_PRIVATE).getString(KEY_THEME, "blue") ?: "blue"
  fun setTheme(ctx: Context, v: String) =
    ctx.getSharedPreferences(PREF, Context.MODE_PRIVATE).edit().putString(KEY_THEME, v).apply()
  ...
  fun saveModules(ctx: Context, list: List<ModuleRecord>) {
    ...
    list.forEach {
      val o = JSONObject()
      o.put("ssid", it.ssid)
      it.customName?.let { v -> o.put("customName", v) }
      it.disjoncteur?.let { v -> o.put("disjoncteur", v) }

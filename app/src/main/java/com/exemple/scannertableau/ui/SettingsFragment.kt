...
class SettingsFragment : Fragment() {
  ...
  override fun onViewCreated(v: View, s: Bundle?) {
    ...
    val sp = v.findViewById<Spinner>(R.id.spTheme)
    val items = listOf("Bleu","Vert","Sombre")
    sp.adapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_dropdown_item, items)
    sp.setSelection( when(Storage.getTheme(requireContext())){ "green"->1; "dark"->2; else->0 } )
    v.findViewById<Button>(R.id.btnApplyTheme).setOnClickListener {
      val sel = when(sp.selectedItemPosition){ 1->"green"; 2->"dark"; else->"blue" }
      Storage.setTheme(requireContext(), sel)
